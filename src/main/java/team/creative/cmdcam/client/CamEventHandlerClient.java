...
public class CamEventHandlerClient {
        ...
        if (mc.player != null && mc.level != null) {
            if (!mc.isPaused()) {
                if (CMDCamClient.getCurrentPath() == null) {
                    ...
                    // mc.player.sendMessage(new StringTextComponent("Registered " +
                    // CMDCamClient.points.size() + ". Point!"), Util.NIL_UUID);
                    // }
                ...
                // mc.player.sendMessage(new StringTextComponent("Cleared all registered
                // points!"), Util.NIL_UUID);
                // }
            }
    ...
    public void worldRender(RenderWorldLastEvent event) {
        ...
        if (CMDCamClient.getCurrentPath() == null && shouldRender && CMDCamClient.points.size() > 0) {
            ...
            for (int i = 0; i < CMDCamClient.points.size(); i++) {
                ...
                DebugRenderer.renderFloatingText((i + 1) + "", point.x + view.x(), point.y + 0.2 + view.y(),
                        point.z + view.z(), -1);
                DebugRenderer.renderFilledBox(point.x - 0.05, point.y - 0.05, point.z - 0.05, point.x + 0.05,
                        point.y + 0.05, point.z + 0.05, 1, 1, 1, 1);
            ...
            for (Iterator<CamInterpolation> iterator = CamInterpolation.interpolationTypes.values().iterator(); iterator
                    .hasNext();) {
    ...
    public void renderMovement(MatrixStack mat, CamInterpolation movement, ArrayList<CamPoint> points) {
        ...
        for (int i = 0; i < steps; i++) {
            ...
            bufferbuilder.vertex(mat.last().pose(), (float) pos.x, (float) pos.y, (float) pos.z)
                    .color((float) color.x, (float) color.y, (float) color.z, 1).endVertex();
        }
        bufferbuilder
                .vertex(mat.last().pose(), (float) points.get(points.size() - 1).x,
                        (float) points.get(points.size() - 1).y, (float) points.get(points.size() - 1).z)
                .color((float) color.x, (float) color.y, (float) color.z, 1).endVertex();
        ...
        if (event instanceof EntityInteract) {
            ...
            event.getPlayer().sendMessage(
                    new StringTextComponent(
                            "Target is set to " + ((EntityInteract) event).getTarget().getStringUUID() + "."),
                    Util.NIL_UUID);
